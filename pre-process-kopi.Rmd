...
```{r}
pacman::p_load(readbulk, tidyverse, ggplot2, ggpubr, cluster, factoextra)
```
...
#ordforråd u. lyd
round(pastecs::stat.desc(Final$log_ord, basic = FALSE, norm = TRUE), digits = 2)
ggplot(Final, aes(x = log_ord))+
  geom_histogram() +
  theme_minimal()
#ordforråd m. lyd
round(pastecs::stat.desc(Final$log_lydord, basic = FALSE, norm = TRUE), digits = 2)
ggplot(Final, aes(x = log_lydord))+
  geom_histogram() +
  theme_minimal()
#forfattergenkendelse
round(pastecs::stat.desc(Final$log_forfatter, basic = FALSE, norm = TRUE), digits = 2)
ggplot(Final, aes(x = log_forfatter))+
  geom_histogram() +
  theme_minimal()
```
...
# Runnings a Spearman's Rho which is one of the non-parametric tests to use if the variables are not normally distributed.
```
...
#Silhouette Plot
plot(silhouette(cutree(Final.hclust,3), distance))
